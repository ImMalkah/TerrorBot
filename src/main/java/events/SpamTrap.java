...
public class SpamTrap extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        List<Message> history = event.getTextChannel().getIterableHistory().complete().stream().limit(7) // gets last 10 messages in channel
                .filter(msg -> !msg.equals(event.getMessage())).toList(); // filters out the last message sent (does not include it)
        int spam = history.stream()
                ...
                .filter(msg -> (event.getMessage().getTimeCreated().toEpochSecond() - msg.getTimeCreated().toEpochSecond() < 6)).toList().size();
